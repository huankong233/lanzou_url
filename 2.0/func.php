...
//获取ID
function GetUrlID($content, $start, $end)
{
...
//请求状态
function check_status($info, $pass): array
{
    ...
    } else if (strpos($info, '输入密码') && empty($pass)) {
        $Json = array(
            'code' => 202,
            'msg' => '请输入密码',
        );
    } else if (strpos($info, '输入密码') && isset($pass)) {
        $Json = array(
            'code' => 203,
            'msg' => '密码已输入',
        );
